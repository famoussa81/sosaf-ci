...
function Lightbox({ product, index, onClose, onPrev, onNext }) {
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <button onClick={onClose} aria-label="Fermer" style={{ position: 'absolute', top: 24, right: 24, width: 44, height: 44, borderRadius: '50%', background: 'oklch(from var(--ivory) l c h / 0.1)', border: '1px solid var(--border-on-dark)', color: '#fff', cursor: 'pointer' }}>
        <Icon name="x" size={20} />
      </button>
      <div onClick={e => e.stopPropagation()} className="lightbox-row" style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 16 }}>
        <button onClick={onPrev} aria-label="Précédent" style={{ flex: '0 0 auto', width: 48, height: 48, borderRadius: '50%', border: '1px solid var(--border-on-dark)', background: 'oklch(from var(--canopy) l c h / 0.6)', color: '#fff', cursor: 'pointer' }}>
        ...
        <button onClick={onNext} aria-label="Suivant" style={{ flex: '0 0 auto', width: 48, height: 48, borderRadius: '50%', border: '1px solid var(--border-on-dark)', background: 'oklch(from var(--canopy) l c h / 0.6)', color: '#fff', cursor: 'pointer' }}>
